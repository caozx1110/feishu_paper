...
def run_unittest_discovery(directory):
    """运行unittest发现模式的测试"""
    print(f"\n{'='*60}")
    print(f"🔍 运行unittest发现模式: {directory}")
    print(f"{'='*60}")
    ...
    try:
        ...
        print(f"⏰ unittest发现模式 - 超时")
        return False
    except Exception as e:
        print(f"💥 unittest发现模式 - 异常: {e}")
        return False
...
def main():
    ...
    # 额外说明
    print(f"\n💡 测试说明:")
    print(f"  - autopaper单元测试: autopaper包的核心功能测试")
    print(f"  - 集成测试: 使用真实环境变量的完整功能测试")
    print(f"  - 遗留功能测试: 项目早期的基础功能测试")
    print(f"  - 其他测试: 项目根目录的其他测试文件")

    print(f"\n🔧 测试环境要求:")
    print(f"  - 需要.env文件配置飞书API凭据")
    print(f"  - 集成测试需要真实的网络连接")
    print(f"  - 某些测试可能因为API限制而失败")
